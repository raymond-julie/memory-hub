...
(function () {
  var knownMemoryIds = new Set();
  var memoryPollTimer = null;
  var memoryPanelCollapsed = false;

  var memoryPanel = document.getElementById("memory-panel");
  var memoryList = document.getElementById("memory-list");
  var memoryCount = document.getElementById("memory-count");
  ...
  if (memoryToggle) {
    memoryToggle.addEventListener("click", function () {
      ...
      memoryPanel.classList.toggle("collapsed", memoryPanelCollapsed);
    });
  }
  ...
  function renderMemoryCard(mem, isNew) {
    ...
    if (mem.scope) {
      ...
      badge.className = "memory-scope-badge scope-" + mem.scope;
      badge.textContent = mem.scope;
    ...
    if (mem.weight != null) {
      ...
      weight.textContent = "w:" + mem.weight;
    ...
    if (mem.created_at) {
      ...
      time.textContent = relativeTime(mem.created_at);
      time.title = mem.created_at;
    ...
    fetch("/v1/memories?limit=30")
      ...
      .then(function (data) {
        ...
        for (var i = 0; i < memories.length; i++) {
          var mem = memories[i];
          var isNew = !knownMemoryIds.has(mem.id);
          memoryList.appendChild(renderMemoryCard(mem, isNew));
        }

        // Update known set after render so first load doesn't flash
        if (knownMemoryIds.size > 0) {
          for (var j = 0; j < memories.length; j++) {
            knownMemoryIds.add(memories[j].id);
          }
        } else {
          // First load: mark all as known (no flash)
          for (var k = 0; k < memories.length; k++) {
            knownMemoryIds.add(memories[k].id);
          }
          // Remove the "new" class since this is the initial load
          var cards = memoryList.querySelectorAll(".memory-card.new");
          for (var c = 0; c < cards.length; c++) {
            cards[c].classList.remove("new");
          }
        }
      })
      .catch(function (err) {
        console.warn("Memory poll failed:", err);
      });
  }
  ...
  document.addEventListener("DOMContentLoaded", startMemoryPolling);
